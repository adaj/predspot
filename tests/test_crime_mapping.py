...
@pytest.mark.parametrize('tfreq,expected', [('M', 'ME'), ('m', 'ME'), ('ME', 'ME'),
                                            ('W', 'W'), ('D', 'D'), ('daily', 'D')])
...
def test_create_gridpoints_errors(study_area):
    ...
    sliver = gpd.GeoDataFrame(geometry=[Polygon([(-35.25, -5.90), (-35.20, -5.85), (-35.25, -5.80), (-35.30, -5.85)])],
                              crs='EPSG:4326')
...
def test_create_gridhexagonal(study_area):
    ...
    inside = [geom.contains(pt) for geom, pt in
              zip(grid.geometry, __import__('geopandas').points_from_xy(grid.lon, grid.lat))]
    assert all(inside)
